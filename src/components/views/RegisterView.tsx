...
export const RegisterView: React.FC<{ onNavigate: (v: string) => void }> = ({ onNavigate }) => {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    clearAuthError();
    setLoading(true);
    const success = await register(name, email, level, password);
    setLoading(false);
    if (success) {
    ...
    <div id="register-page" className="max-w-md mx-auto px-4 py-16">
      <div className="card-ai-luxury p-8 space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <h2 className="text-2xl font-black text-[#134E4A]">Create Your Account</h2>
          <p className="text-xs text-teal-900/80 font-medium">Start practicing English with your personal AI partner</p>
        </div>
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          <div>
            <label className="block text-xs font-bold uppercase text-teal-800/80 mb-1">Full Name</label>
            <div className="relative">
              <User className="w-4 h-4 text-teal-700/60 absolute left-3.5 top-3.5" />
              <input
                type="text"
                value={name}
          ...
          <div>
            <label className="block text-xs font-bold uppercase text-teal-800/80 mb-1">Password</label>
            <div className="relative">
          ...
          <div>
            <label className="block text-xs font-bold uppercase text-teal-800/80 mb-1">Current English Level</label>
            <select
              value={level}
              onChange={(e) => setLevel(e.target.value)}
              className="w-full px-4 py-3 rounded-2xl border border-[#CBDED9] bg-[#DCEDE9] text-[#134E4A] text-sm outline-none font-medium focus:ring-2 focus:ring-[#0F766E]"
            >
              <option value="Beginner">Beginner Level (Free)</option>
              <option value="Intermediate">Intermediate Level (Fluency Focus)</option>
              <option value="Advanced">Advanced Level (Mastery Focus)</option>
            </select>
          </div>
