...
export const CurriculumHubView: React.FC<CurriculumHubViewProps> = ({ onNavigate }) => {
  ...
  return (
    <div id="curriculum-hub-page" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      {/* Header Banner */}
      <div className="p-8 sm:p-10 rounded-3xl bg-gradient-to-r from-[#042F2C] via-[#0F766E] to-[#0D9488] text-white shadow-xl space-y-4 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-80 h-80 bg-[#14B8A6]/20 rounded-full blur-3xl pointer-events-none" />

        <div className="inline-flex items-center space-x-2 px-3.5 py-1 rounded-full bg-white/10 border border-white/20 text-teal-100 text-xs font-extrabold">
        ...
        <p className="text-teal-100/90 text-xs sm:text-sm max-w-2xl leading-relaxed font-medium">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2 max-w-3xl">
          <div className="p-3.5 rounded-2xl bg-white/10 border border-white/10 backdrop-blur-sm flex items-center space-x-3">
            <span className="p-2 rounded-xl bg-teal-400/20 text-teal-200 font-black text-xs">FREE</span>
            <div>
              <p className="text-xs font-extrabold text-white">200 Beginner Topics</p>
              <p className="text-[10px] text-teal-100/70">Everyday • Pictures • Mini Games</p>
            </div>
          </div>

          <div className="p-3.5 rounded-2xl bg-white/10 border border-white/10 backdrop-blur-sm flex items-center space-x-3">
            <span className="p-2 rounded-xl bg-[#F59E0B]/20 text-[#F59E0B] font-black text-xs">PRO</span>
            <div>
              <p className="text-xs font-extrabold text-white">100 Intermediate Topics</p>
              <p className="text-[10px] text-teal-100/70">Debates • Idioms • Case Studies</p>
            </div>
          </div>

          <div className="p-3.5 rounded-2xl bg-white/10 border border-white/10 backdrop-blur-sm flex items-center space-x-3">
            <span className="p-2 rounded-xl bg-[#14B8A6]/20 text-[#14B8A6] font-black text-xs">PRO</span>
            <div>
              <p className="text-xs font-extrabold text-white">100 Advanced Topics</p>
              <p className="text-[10px] text-teal-100/70">Business • Academic • Ethics • AI</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 border-b border-[#CBDED9] pb-4">
        
        <div className="flex items-center space-x-2 bg-[#DCEDE9] p-1.5 rounded-2xl border border-[#CBDED9]">
          {(['Beginner', 'Intermediate', 'Advanced'] as EnglishLevel[]).map((level) => {
            ...
            return (
              ...
              >
                ...
                {isFree ? (
                  <span className="text-[9px] px-1.5 py-0.5 rounded bg-[#DCEDE9] text-[#0F766E] font-black border border-[#CBDED9]">FREE</span>
                ) : (
                  <span className="text-[9px] px-1.5 py-0.5 rounded bg-amber-100 text-amber-900 font-extrabold flex items-center space-x-0.5">
                    <Zap className="w-2.5 h-2.5 text-[#F59E0B] fill-[#F59E0B]" />
                    <span>PRO</span>
                  </span>
                )}
              </button>
        ...
        <div className="relative w-full sm:w-72">
          <Search className="w-4 h-4 text-teal-800/60 absolute left-3.5 top-3" />
          <input
            type="text"
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder="Search topic or keyword..."
            className="w-full pl-10 pr-4 py-2.5 rounded-2xl bg-[#E6F1EF] border border-[#CBDED9] text-xs text-[#134E4A] font-medium focus:outline-none focus:ring-2 focus:ring-[#0F766E]"
          />
        </div>
      ...
      <div className="flex items-center space-x-2 overflow-x-auto pb-2 scrollbar-none">
        <span className="text-xs font-extrabold text-teal-800/70 uppercase tracking-wider shrink-0 flex items-center space-x-1 mr-1">
        ...
        {filteredTopics.slice(0, 30).map((topic) => {
          ...
          return (
            <div
              ...
              className="p-6 rounded-3xl bg-[#E6F1EF] border border-[#CBDED9] hover:border-[#0F766E] shadow-sm hover:shadow-lg transition-all duration-200 flex flex-col justify-between space-y-4 cursor-pointer group relative overflow-hidden"
            >
              <div className="space-y-3">
                
                <div className="flex items-center justify-between">
                  <span className="text-[11px] font-black px-2.5 py-1 rounded-full bg-[#DCEDE9] text-[#0F766E] border border-[#CBDED9]">
                    Topic #{topic.topicNumber}
                  </span>

                  {topic.isPremium ? (
                    <span className="text-[10px] font-extrabold px-2 py-0.5 rounded-full bg-amber-100 text-amber-900 border border-amber-300 flex items-center space-x-1">
                      <Lock className="w-3 h-3 text-[#D97706]" />
                      <span>Premium</span>
                    </span>
                  ) : (
                    <span className="text-[10px] font-extrabold px-2 py-0.5 rounded-full bg-[#DCEDE9] text-[#0F766E] border border-[#CBDED9]">
                      Free
                    </span>
                  )}
                </div>
                ...
                <p className="text-xs text-teal-900/80 font-medium line-clamp-2 leading-relaxed">
                  {topic.description}
                </p>
                ...
                <div className="pt-1 flex flex-wrap gap-1.5 text-[10px]">
                  {topic.level === 'Beginner' ? (
                    <>
                      <span className="px-2 py-0.5 rounded bg-[#DCEDE9] text-[#134E4A] font-bold">10 Verbs</span>
                      <span className="px-2 py-0.5 rounded bg-[#DCEDE9] text-[#134E4A] font-bold">10 Adjectives</span>
                      <span className="px-2 py-0.5 rounded bg-[#DCEDE9] text-[#0F766E] font-bold">Picture Description</span>
                      <span className="px-2 py-0.5 rounded bg-[#DCEDE9] text-[#0F766E] font-bold">Mini Games</span>
                    </>
                  ) : (
                    <>
                      <span className="px-2 py-0.5 rounded bg-[#DCEDE9] text-[#0F766E] font-bold">5 {topic.level === 'Advanced' ? 'Academic' : 'Advanced'} Vocab</span>
                      <span className="px-2 py-0.5 rounded bg-amber-100 text-amber-900 font-bold">5 Idioms</span>
                      <span className="px-2 py-0.5 rounded bg-[#DCEDE9] text-[#134E4A] font-bold">Debates & Case Studies</span>
                    </>
                  )}
                </div>
